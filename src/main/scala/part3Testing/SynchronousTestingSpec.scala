...
class SynchronousTestingSpec extends WordSpecLike with BeforeAndAfterAll {
  ...
  "A counter" should {
    "synchronously increase its counter" in {
      val counter = TestActorRef[Counter](Props[Counter]) // to make test synchronous. needs implicit actorSystem
    ...
    "synchronously increase its counter at the call of the receive function" in {
      ...
      counter.receive(Inc) // the same as above, but testActorRef can call receive method
      assert(counter.underlyingActor.count == 1)
    }

    "work on the calling thread dispatcher" in {
      val counter = system.actorOf(Props[Counter].withDispatcher(CallingThreadDispatcher.Id)) // to make test synchronous
      val probe = TestProbe()
  ...
  // we should always use asynchronous test because most times we need asynchronously to a good communication between actors
  }
}
