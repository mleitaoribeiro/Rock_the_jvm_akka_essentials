...
object Mailboxes extends App {
  ...
  // attach the mailbox to a dispatcher
...
//  supportTicketLogger ! "[P1] do this when you have the time"
  ...
  // after which time can I be send another message and be prioritized accordingly?
  // we don't know and we cannot configure the wait

  /**
   *  Interesting case #2 - control aware mailbox
   *  we'll UnboundedControlAwareMailbox
   *  management messages are special
   */
  // step 1 - mark important messages as control messages
  case object ManagementTicket extends ControlMessage

  // step2 - configure who gets the mailbox
