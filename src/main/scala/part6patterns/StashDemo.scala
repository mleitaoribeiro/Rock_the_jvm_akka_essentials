...
object StashDemo extends App {
  ...
  /*
    Resource Actor - has access to a resource and it's block by it
    ...
    [Read, Open, Write]
      - stash Read
        Stash: [Read]
        (stash - special memory zone, when we stash we put the messages in a special queue and not in the mailbox queue)
      - open => switch to the open state
        Mailbox: [Read, Write]
  ...
  */
  ...
  resourceActor ! Open // stashed because in Open we cannot Open again
